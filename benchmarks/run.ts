...
async function benchFramework(
  ...
  for (const scenario of scenarios) {
    ...
    results.push(result);
    console.log(
      ` ${fmt(result.requestsPerSec)} req/s  (p50: ${fmt(result.latencyP50, 1)}ms, p99: ${fmt(result.latencyP99, 1)}ms)`,
    );
  }
...
function printComparisonTable(all: FrameworkResults[]): void {
  ...
  for (const scenario of scenarios) {
    ...
    for (const r of rows) {
      console.log(
        `| ${pad(r.fw, 12)} | ${pad(fmt(r.requestsPerSec), 10, false)} | ${pad(fmt(r.latencyP50, 1), 8, false)} | ${pad(fmt(r.latencyP99, 1), 8, false)} | ${pad(`${fmt(r.throughputMBps, 1)} MB/s`, 10, false)} |`,
      );
    }
    console.log("");
  }
