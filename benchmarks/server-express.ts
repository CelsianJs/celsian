...
export async function startExpressServer(port: number): Promise<{ close: () => Promise<void> }> {
  ...
  const mw2 = (_req: Request, res: Response, next: NextFunction) => {
    res.setHeader("x-mw-2", "true");
    next();
  };
  const mw3 = (_req: Request, res: Response, next: NextFunction) => {
    res.setHeader("x-mw-3", "true");
    next();
  };
  const mw4 = (_req: Request, res: Response, next: NextFunction) => {
    res.setHeader("x-mw-4", "true");
    next();
  };
  const mw5 = (_req: Request, res: Response, next: NextFunction) => {
    res.setHeader("x-mw-5", "true");
    next();
  };
