...
export async function startHonoServer(port: number): Promise<{ close: () => Promise<void> }> {
  ...
  app.use('/hooks', async (c, next) => { c.header('x-hook-1', 'true'); await next(); });
  app.use('/hooks', async (c, next) => { c.header('x-hook-2', 'true'); await next(); });
  app.use('/hooks', async (c, next) => { c.header('x-hook-3', 'true'); await next(); });
