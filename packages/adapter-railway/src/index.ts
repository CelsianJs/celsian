...
export function railwayAdapter(options: RailwayAdapterOptions = {}): DeployAdapter {
  ...
  return {
    ...
    async buildEnd({ serverEntry, outDir }) {
      const startCommand = `node ${shellQuote(serverEntry)}`;
      // 1. Generate Procfile — Railway uses this to start the app
      const procfile = `web: ${startCommand}\n`;
      ...
      if (generateDockerfile) {
        const dockerfile = generateRailwayDockerfile(nodeVersion, serverEntry, healthCheckPath);
...
function generateRailwayDockerfile(nodeVersion: string, serverEntry: string, healthCheckPath: string): string {
...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \\
  CMD wget -qO- http://localhost:3000${healthCheckPath} || exit 1
