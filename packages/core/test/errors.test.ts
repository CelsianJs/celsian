...
describe("ValidationError", () => {
  ...
  it("should handle issues without a path", () => {
    const err = new ValidationError([{ message: "is required" }]);
  ...
  it("should include human-readable message in toJSON()", () => {
    const err = new ValidationError([{ message: "required field", path: ["email"] }]);
    const json = err.toJSON();
  ...
  it("should handle numeric path segments", () => {
    const err = new ValidationError([{ message: "invalid value", path: ["items", 0, "name"] }]);
...
describe("assertPlugin", () => {
  ...
  it("should suggest the correct usage pattern", () => {
    expect(() => assertPlugin("oops")).toThrow(/Usage: app\.register\(async \(app, opts\) => \{ \/\* \.\.\. \*\/ \}\)/);
  });
});
...
describe("assertDecorationUnique", () => {
  it("should throw showing both existing and new values", () => {
    ...
    expect(() => assertDecorationUnique("db", "postgres-pool", "mysql-pool")).toThrow(/Decoration "db" already exists/);
    expect(() => assertDecorationUnique("db", "postgres-pool", "mysql-pool")).toThrow(/Existing value: postgres-pool/);
    expect(() => assertDecorationUnique("db", "postgres-pool", "mysql-pool")).toThrow(/new value: mysql-pool/);
  });
  ...
  it("should format object values as JSON", () => {
    expect(() => assertDecorationUnique("config", { a: 1 }, { b: 2 })).toThrow(/Existing value: \{"a":1\}/);
  });

  it("should suggest using a unique name", () => {
    expect(() => assertDecorationUnique("x", 1, 2)).toThrow(/Use a unique name or remove the conflicting plugin/);
  });
});
...
describe("app.register() with non-function", () => {
  it("should throw CelsianError when registering a string", async () => {
    ...
    await expect(app.register("not-a-plugin" as any)).rejects.toThrow(/app\.register\(\) expects a plugin function/);
  });
