...
import { compileRoute, compileRoutes, matchRoute, applyRewrite } from '../src/match.js';
import { isOriginAllowed, corsHeaders, preflightResponse } from '../src/cors.js';
import { createEdgeRouter, getProjectId } from '../src/router.js';
import type { RouteEntry, CorsConfig } from '../src/types.js';
...
describe('compileRoute', () => {
  ...
  it('compiles a pattern with multiple params', () => {
    const entry: RouteEntry = { pattern: '/orgs/:orgId/repos/:repoId', methods: ['GET'], origin: 'http://localhost:3000' };
    const compiled = compileRoute(entry);
...
describe('createEdgeRouter', () => {
  ...
  it('updates routes via POST /__routes', async () => {
    const req = new Request('https://edge.example.com/__routes', {
      ...
      body: JSON.stringify({
        routes: [
          { pattern: '/v2/health', methods: ['GET'], origin: 'http://new-svc:4000' },
        ],
      }),
    });
    const res = await router.fetch(req);
...
describe('matchRoute — URL-decoded params', () => {
  it('decodes URI-encoded param values', () => {
    const routes = compileRoutes([
      { pattern: '/files/:name', methods: ['GET'], origin: 'http://localhost:3000' },
    ]);
