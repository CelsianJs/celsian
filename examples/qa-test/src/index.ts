...
import { createApp, cors, HttpError } from '@celsian/core';
...
await app.register(cors({
  origin: ['http://localhost:3456', 'http://example.com'],
  credentials: true,
  maxAge: 3600,
...
app.addHook('preParsing', (req, reply) => {
...
app.addHook('preSerialization', (req, reply) => {
...
// 7. Cookie handling
app.get('/api/cookies/set', (req, reply) => {
...
app.get('/api/cookies/clear', (req, reply) => {
...
app.post('/api/auth/login', async (req, reply) => {
  ...
  const token = await jwtInstance.sign(
    { sub: body?.username ?? 'anonymous', role: 'user' },
    { expiresIn: '1h' }
  );
...
// ─── Start HTTP Server manually (workaround for serve() ESM bug) ───
