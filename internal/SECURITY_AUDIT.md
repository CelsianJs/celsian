...
**Date:** 2026-03-08 (updated 2026-03-26, corrected 2026-07-26)
...
**This document previously recorded C-2 and C-3 (path traversal in `reply.sendFile()`
and `reply.download()`) as "Fix Applied". That was wrong, and the wrong entries stood
for four months.**

The recorded fix was "added `resolve()` to normalize the path". `resolve()` normalizes
a path; it does not confine one. `resolve('/uploads/../../etc/hosts')` is `/etc/hosts`,
which is exactly the traversal the finding described. Verified again on 2026-07-26
against the current source:

```
sendFile (no root):   status=200  *** ARBITRARY FILE READ ***
download (no root):   status=200  *** ARBITRARY FILE READ ***
sendFile (with root): status=403  (correctly blocked)
```

Only the `reply.sendFile(path, { root })` form is safe. It resolves against `root` and
returns 403 when the result escapes (`packages/core/src/reply.ts:161-177`). `download()`
has no `root` option at all, so it has no confinement mechanism.

C-2 and C-3 are reopened below as OPEN. The "Overall Security Posture: STRONG" line was
also removed, because it was derived from those two entries being closed.

Why this mattered more than the bug itself: a documented guarantee that the code does not
provide is worse than a known gap. It stops the next reader from looking. Anyone auditing
this framework between 2026-03 and 2026-07 would have read "Fix Applied" and moved on.

---

## Summary

The CelsianJS framework demonstrates a generally security-conscious design with several strong patterns (production error sanitization, request timeouts, body size limits, use of `jose` for JWT). The initial audit identified **3 CRITICAL**, **5 HIGH**, **6 MEDIUM**, **4 LOW**, and **5 INFO** findings.

**Current state: C-1 is fixed. C-2 and C-3 are OPEN (see the correction above). All HIGH
issues are resolved.** The v0.2.0 hardening sprint addressed remaining MEDIUM and LOW
findings, added CSRF protection, and hardened the adapter-node path traversal prevention.
